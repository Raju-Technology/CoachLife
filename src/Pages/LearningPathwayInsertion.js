...
function LearningPathwayInsertion() {
    ...
    const updatePlayerData = async (data) => {
        const req = await fetch("https://ob9i4c7s3i.execute-api.ap-south-1.amazonaws.com/CoachLife/CL_Adding_Pathway", {
            method: 'post',
        ...
        <div>
            ...
            <m.section className="py-[120px] lg:py-[120px] md:py-[75px] sm:py-[50px] xs:py-[80px] xxs:py-[50px]"  {...fadeIn}>
                <Container>
                    <Row>
                        <Col className='mb-[6%]'>
                            <h6 className="font-serif text-gray-500 text-center font-medium mb-[25px] lg:mb-[15px]">LearningPathway Insertion</h6>
                        </Col>
                    </Row>
                    <Row className="justify-center">
                        <Col>
                            <Formik
                                initialValues={{
                                    activity: '', level: '', stage: '', topic: '', terms_condition: false, type: '', time: '',
                                    preReq: '', defaultPoints: ''
                                }}
                                ...
                                onSubmit={async (values, actions) => {
                                    ...
                                    response.message === "Learning pathway data inserted successfully." && resetForm(actions)
                                }}
                            >
                                {({ isSubmitting, status }) => (
                                    <Form>
                                        <Row className="row-cols-1 row-cols-md-2">
                                            <Col className="mb-16 lg:mb-[25px] sm:mb-0">
                                                <Input showErrorMsg={false} type="text" name="activity" className="py-[15px] px-[20px] text-md w-full border-[1px] border-solid border-[#dfdfdf]" labelClass="mb-[25px]" placeholder="Activity" />
                                                ...
                                                <Input showErrorMsg={false} type="text" name="topic" className="py-[15px] px-[20px] w-full text-md border-[1px] border-solid border-[#dfdfdf]" labelClass="mb-[25px]" placeholder="Topic" />
                                            </Col>
                                            <Col className="mb-16 lg:mb-[25px]">
                                                <Input showErrorMsg={false} type="text" name="type"  className="py-[15px] px-[20px] w-full text-md border-[1px] border-solid border-[#dfdfdf]" labelClass="mb-[25px]" placeholder="Type" />
                                                <Input showErrorMsg={false} type="text" name="time" className="py-[15px] px-[20px] w-full text-md border-[1px] border-solid border-[#dfdfdf]" labelClass="mb-[25px]" placeholder="Time" />
                                                <Input showErrorMsg={false} type="text" name="preReq" className="py-[15px] px-[20px] w-full text-md border-[1px] border-solid border-[#dfdfdf]" labelClass="mb-[25px]" placeholder="PreRequisites" />
                                                <Input showErrorMsg={false} type="text" name="defaultPoints" className="py-[15px] px-[20px] w-full text-md border-[1px] border-solid border-[#dfdfdf]" labelClass="mb-[25px]" placeholder="Default Points" />
                                            </Col>
                                            ...
                                            <Col className="text-right sm:text-center">
                                                <Buttons ariaLabel="form button" type="submit" className={`text-xs tracking-[1px] rounded-none py-[12px] px-[28px] uppercase${isSubmitting ? " loading" : ""}`} themeColor={["#556fff", "#ff798e"]} size="md" color="#fff" title="Pathway Insertion" />
                                            </Col>
                                        </Row>
                                        <AnimatePresence>
                                            {status && <Row><Col xs={12}><m.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><MessageBox className="mt-[20px] py-[10px]" theme="message-box01" variant="success" message="Player has been Created Successfully!" /></m.div></Col></Row>}
                                        </AnimatePresence>
                                    </Form>
                                )}
                            </Formik>
...
export default LearningPathwayInsertion
