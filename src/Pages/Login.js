...
function Login(props) {
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      const decryptedPassword = encryptPassword(values.password, 3); // Use the same shift value as in encryption
      ...
      props.setName(payload.UserName)

      const response = await fetch("https://tlzwpdr34g.execute-api.ap-south-1.amazonaws.com/CoachLife/Login", {
        method: "POST",
          ...
          <Container>
            <Row className="full-screen md:h-[520px] sm:h-[100vh]">
              <Col xl={5} md={5} className="relative flex flex-col justify-center">
                ...
                <h1 style={{ color: 'white', fontSize: '38px' }}>Login</h1>
                <div style={{ width: formWidth, borderRadius: '5px' }}>
                  ...
                  >
                    ...
                    <Form.Item style={{ textAlign: 'right'}}>
                        ...
                        <Button type="primary" style={{ background: '#0294CF', border: 'none', marginLeft:'5px' }} onClick={handleRegisterClick}>
                          New User
                        </Button>
                    </Form.Item>
